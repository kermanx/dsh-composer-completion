...
export class CompletionMemoryCache {
    ...
    clearSession(sessionId) {
        for (let index = this.entries.length - 1; index >= 0; index -= 1) {
            if (this.entries[index]?.sessionId === sessionId)
    ...
    find(sessionId, anchorMessageId, model, promptVersion, draft) {
        ...
        for (const entry of this.entries) {
            ...
            if (!reusable || (winner !== undefined && winner.draft.length >= entry.draft.length))
                continue;
            winner = entry;
        }
        if (winner === undefined)
    ...
    put(sessionId, context, draft, completion) {
        ...
        while (this.entries.length > this.limit) {
            ...
            for (let index = 1; index < this.entries.length; index += 1) {
                if ((this.entries[index]?.usedAt ?? 0) < (this.entries[oldest]?.usedAt ?? 0))
                    oldest = index;
            }
    ...
    prune(now) {
        for (let index = this.entries.length - 1; index >= 0; index -= 1) {
            if ((this.entries[index]?.expiresAt ?? 0) <= now)
                this.entries.splice(index, 1);
        }
    }
}
...
function project(baseDraft, completion, draft) {
    ...
    if (typed.startsWith(completion))
        return { kind: 'waiting' };
    return { kind: 'diverged' };
}
...
export class CompletionStore {
    ...
    update(environment) {
        ...
        if (previous !== undefined && previous.sessionId !== environment.sessionId)
            this.reset(previous.sessionId);
        if (environment.running && !this.previousRunning)
            this.resetConversation(environment.sessionId);
    ...
    dismiss() {
        if (this.snapshot.suggestion === null)
            return false;
        this.cancelTimer();
        this.abortInflight();
    ...
    async start(generation, requested) {
        ...
        if (current === undefined || !current.eligible || current.running
            || current.sessionId !== requested.sessionId || current.draft !== requested.draft)
            return;
        ...
        try {
            for await (const frame of this.remote.complete({
                ...
                if (this.inflight !== inflight || abort.signal.aborted)
                    return;
                if (context !== undefined
                    && (context.anchorMessageId !== frame.context.anchorMessageId
                        || context.model !== frame.context.model
                        || context.promptVersion !== frame.context.promptVersion))
                    return;
                ...
                if (live === undefined || !live.eligible || live.running || live.sessionId !== requested.sessionId)
                    return;
        ...
        const live = this.environment;
        if (live === undefined || live.sessionId !== requested.sessionId)
            return;
    ...
    publish(suggestion) {
        if (this.snapshot.suggestion === suggestion)
            return;
...
//# sourceMappingURL=completion-store.js.map