...
import { buildCompletionPrompt, completionAnchor, PROMPT_VERSION, STOP_SEQUENCES, SYSTEM_PROMPT, } from "./prompt.js";
class CompletionProtocolDecoder {
    ...
    push(delta, maxCharacters) {
        if (this.mode === 'none' || this.mode === 'closed' || this.mode === 'invalid')
            return undefined;
        this.raw += delta;
        const candidate = this.raw.trimStart();
        if (candidate === '')
            return undefined;
        if (CompletionProtocolDecoder.NONE.startsWith(candidate))
            return undefined;
        if (candidate.startsWith(CompletionProtocolDecoder.NONE)) {
            this.mode = 'none';
            return '';
        }
        if (CompletionProtocolDecoder.OPEN.startsWith(candidate))
            return undefined;
    ...
    finish(maxCharacters) {
        ...
        if (projected !== undefined)
            return projected;
        if (this.mode !== 'completion' && this.mode !== 'closed')
            return '';
        const candidate = this.raw.trimStart();
        const content = candidate.slice(CompletionProtocolDecoder.OPEN.length);
        const close = content.indexOf(CompletionProtocolDecoder.CLOSE);
    ...
    closingPrefixLength(text) {
        ...
        for (let length = limit; length > 0; length -= 1) {
            if (CompletionProtocolDecoder.CLOSE.startsWith(text.slice(-length)))
                return length;
        }
        return 0;
    }
    ...
    async *complete(request, signal) {
        ...
        for await (const chunk of this.ctx.llm.stream(options)) {
            if (requestSignal.aborted)
                return;
            if (completionAnchor(agent.session) !== prompt.anchorMessageId)
                return;
